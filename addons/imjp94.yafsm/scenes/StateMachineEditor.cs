...
[Tool]
public class StateMachineEditor : "res://addons/imjp94.yafsm/scenes/flowchart/FlowChart.gd"
{
	 
	public const var StateMachinePlayer = GD.Load("../src/StateMachinePlayer.gd");
	...
	public const var StateNode = GD.Load("state_nodes/StateNode.tscn");
	public const var TransitionLine = GD.Load("transition_editors/TransitionLine.tscn");
	public const var StateNodeScript = GD.Load("state_nodes/StateNode.gd");
	public const var StateMachineEditorLayer = GD.Load("StateMachineEditorLayer.gd");
	public const var PathViewer = GD.Load("PathViewer.gd");
	
	[Signal] delegate void InspectorChanged(property);// Inform plugin to refresh inspector
	[Signal] delegate void DebugModeChanged(newDebugMode);
	...
	public onready var contextMenu = GetNode("ContextMenu");
	public onready var stateNodeContextMenu = GetNode("StateNodeContextMenu");
	public onready var convertToStateConfirmation = GetNode("ConvertToStateConfirmation");
	public onready var saveDialog = GetNode("SaveDialog");
	public onready var createNewStateMachineContainer = GetNode("MarginContainer");
	public onready var createNewStateMachine = GetNode("MarginContainer/CreateNewStateMachine");
	public onready var paramPanel = GetNode("ParametersPanel");
	public __TYPE pathViewer = new HBoxContainer()
	public __TYPE conditionVisibility = new TextureButton()
	public __TYPE unsavedIndicator = new Label()
	public __TYPE messageBox = new VBoxContainer()
	
	public __TYPE editorAccentColor = Color.white;
	public __TYPE transitionArrowIcon;
	
	public __TYPE undoRedo;
	
	public __TYPE debugMode = false {set{SetDebugMode(value);}}
	var stateMachinePlayer {set{SetStateMachinePlayer(value);}}
	var stateMachine {set{SetStateMachine(value);}}
	public __TYPE canGuiNameEdit = true;
	public __TYPE canGuiContextMenu = true;
	
	private __TYPE _reconnectingConnection;
	private __TYPE _lastIndex = 0;
	private __TYPE _lastPath = "";
	private __TYPE _messageBoxDict = new Dictionary(){};
	private __TYPE _contextNode;
	private __TYPE _currentState = "";
	private __TYPE _lastStack = new Array(){};
	
	
	public void _Init()
	{  
		pathViewer.mouse_filter = MOUSEFilterIgnore;
		pathViewer.SetScript(PathViewer);
		pathViewer.Connect("dir_pressed", this, "_on_path_viewer_dir_pressed");
	...
	public void _OnStateMachinePlayerChanged(__TYPE newStateMachinePlayer)
	{  
		if(!state_machine_player)
		{
			return;
		}
	...
	public void _OnStateMachineChanged(__TYPE newStateMachine)
	{  
		...
		if(newStateMachine)
		{
			rootLayer.state_machine = stateMachine;
	...
	{  
		if(event is InputEventMouseButton)
		{
			...
			{
				case BUTTONRight:
					if(event.pressed && canGuiContextMenu)
					{
						contextMenu.SetItemDisabled(1, currentLayer.state_machine.HasEntry());
						contextMenu.SetItemDisabled(2, currentLayer.state_machine.HasExit());
	...
	public __TYPE ConvertToStateMachine(__TYPE layer, __TYPE node)
	{  
	...
	public __TYPE ConvertToState(__TYPE layer, __TYPE node)
	{  
	...
	public void ClearGraph(__TYPE layer)
	{  
		ClearConnections();
	...
	public void DrawGraph(__TYPE layer)
	{  
	...
	public void _OnNodeAdded(__TYPE layer, __TYPE newNode)
	{  
	...
	public __TYPE _OnNodeRemoved(__TYPE layer, __TYPE nodeName)
	{  
	...
	public void SetDebugMode(__TYPE v)
	{  
		if(debugMode != v)
		{
			debugMode = v;
			_OnDebugModeChanged(v);
			EmitSignal("debug_mode_changed", debugMode);
	
		}
	}
	
	public void SetStateMachinePlayer(__TYPE smp)
	{  
		if(stateMachinePlayer != smp)
		{
			stateMachinePlayer = smp;
			_OnStateMachinePlayerChanged(smp);
	
		}
	}
	
	public void SetStateMachine(__TYPE sm)
	{  
		if(stateMachine != sm)
		{
			stateMachine = sm;
			_OnStateMachineChanged(sm);
